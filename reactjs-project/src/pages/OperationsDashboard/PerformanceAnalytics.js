...
export default function PerformanceAnalytics() {
    ...
    const fetchOrdersData = useCallback(async (page, status) => {
        try {
            setOrdersLoading(true);
    ...
    const convertStatusForAPI = (status) => {
        const statusMap = {
            'All': 'Tất cả',
            'Pending': 'Chờ xử lý',
    ...
    if (loading) {
        return (_jsx(GlassCard, { className: "flex items-center justify-center h-64", children: _jsx("div", { className: "text-gray-800 text-lg", children: t('dashboard.operations.performance.loadingData', 'Loading performance data...') }) }));
    }
    ...
    return (_jsxs(GlassCard, { className: "space-y-6", children: [error && (_jsxs("div", { className: "bg-yellow-500/30 border border-yellow-400/50 text-yellow-800 p-4 rounded-lg", children: ["\u26A0\uFE0F ", error] })), _jsxs("div", { className: "flex flex-col sm:flex-row sm:items-center justify-between gap-4", children: [_jsx("h2", { className: "text-xl font-semibold text-gray-800", children: t('dashboard.operations.tabs.performance') }), _jsxs("div", { className: "flex flex-col sm:flex-row items-start sm:items-center gap-3 sm:gap-4", children: [_jsx("div", { className: "flex flex-wrap gap-2", children: statusOptions.map((status) => (_jsx(GlassButton, { size: "sm", variant: selectedStatus === status ? 'primary' : 'secondary', onClick: () => handleStatusChange(status), children: getStatusLabel(status) }, status))) }), _jsx(GlassButton, { onClick: handleRefresh, size: "sm", variant: "primary", className: "whitespace-nowrap self-start sm:self-auto", disabled: loading || ordersLoading, children: _jsxs("span", { className: "flex items-center gap-2", children: [_jsx("svg", { xmlns: "http://www.w3.org/2000/svg", viewBox: "0 0 24 24", fill: "none", stroke: "currentColor", strokeWidth: "2", strokeLinecap: "round", strokeLinejoin: "round", className: "w-5 h-5", children: _jsx("path", { d: "M4.93 19.07a10 10 0 1 0 0-14.14M4 4v5h5" }) }), t('common.refresh')] }) })] })] }), _jsx(PerformanceStatCards, { performanceData: performanceData }), _jsx(RecentOrdersTable, { orders: recentOrders }), totalPages > 1 && (_jsxs("div", { className: "flex items-center justify-between mt-6 pt-4 border-t border-gray-200/30", children: [_jsxs("div", { className: "text-sm text-gray-600 font-medium", children: ["Showing ", currentPage * ITEMS_PER_PAGE + 1, "-", Math.min((currentPage + 1) * ITEMS_PER_PAGE, totalElements), " of ", totalElements, " orders"] }), _jsxs("div", { className: "flex items-center gap-1", children: [_jsx(GlassButton, { size: "sm", variant: "secondary", onClick: () => handlePageChange(Math.max(currentPage - 1, 0)), disabled: currentPage === 0, className: `px-3 ${currentPage === 0 ? 'opacity-50 cursor-not-allowed' : 'hover:bg-white/30'}`, children: "\u2190 Previous" }), _jsx("div", { className: "flex items-center gap-1 mx-2", children: Array.from({ length: Math.min(totalPages, 5) }, (_, i) => {
                                    let pageNumber;
                                    if (totalPages <= 5) {
                                        pageNumber = i;
                                    }
                                    ...
                                    return (_jsx(GlassButton, { size: "sm", variant: currentPage === pageNumber ? 'primary' : 'secondary', onClick: () => handlePageChange(pageNumber), className: `min-w-[36px] h-9 ${currentPage === pageNumber
                                            ? 'bg-blue-500/80 text-white font-semibold ring-2 ring-blue-400/50'
                                            : 'hover:bg-white/20'}`, children: pageNumber + 1 }, pageNumber));
                                }) }), _jsx(GlassButton, { size: "sm", variant: "secondary", onClick: () => handlePageChange(Math.min(currentPage + 1, totalPages - 1)), disabled: currentPage === totalPages - 1, className: `px-3 ${currentPage === totalPages - 1 ? 'opacity-50 cursor-not-allowed' : 'hover:bg-white/30'}`, children: "Next \u2192" })] })] }))] }));
}
