...
export default function OperationsDashboard({ user, onLogout }) {
    ...
    const operationsOverviewRef = useRef(null);
    ...
    return (_jsxs("div", { className: "min-h-screen flex bg-gradient-to-br from-purple-100 via-blue-50 to-indigo-100", children: [_jsx("div", { className: "hidden md:block", children: _jsx(Sidebar, { activeTab: tab, onTabChange: tab => setTab(tab), role: "operations" }) }), _jsxs("div", { className: "flex-1 flex flex-col min-w-0 w-full md:w-auto", children: [_jsx(Navbar, { user: user, onLogout: onLogout, title: t('dashboard.operations.title', 'Operations Manager Dashboard'), subtitle: t('dashboard.operations.subtitle', 'Monitor and manage operational metrics') }), _jsx("div", { className: "md:hidden fixed bottom-0 left-0 right-0 bg-white/90 backdrop-blur-lg border-t border-white/30 px-4 py-2 z-50", children: _jsxs("div", { className: "flex justify-around items-center", children: [_jsxs("button", { onClick: () => setTab("overview"), className: `flex flex-col items-center py-2 px-1 ${tab === "overview" ? "text-blue-600" : "text-gray-600"}`, children: [_jsx(FiHome, { className: "text-xl mb-1" }), _jsx("span", { className: "text-xs", children: t('dashboard.operations.tabs.overview', 'Overview') })] }), _jsxs("button", { onClick: () => setTab("performance"), className: `flex flex-col items-center py-2 px-1 ${tab === "performance" ? "text-blue-600" : "text-gray-600"}`, children: [_jsx(FiBarChart2, { className: "text-xl mb-1" }), _jsx("span", { className: "text-xs", children: t('dashboard.operations.tabs.performance', 'Performance') })] }), _jsxs("button", { onClick: () => setTab("monitoring"), className: `flex flex-col items-center py-2 px-1 ${tab === "monitoring" ? "text-blue-600" : "text-gray-600"}`, children: [_jsx(FiActivity, { className: "text-xl mb-1" }), _jsx("span", { className: "text-xs", children: t('dashboard.operations.tabs.monitoring', 'Monitoring') })] }), _jsxs("button", { onClick: () => setTab("staff"), className: `flex flex-col items-center py-2 px-1 ${tab === "staff" ? "text-blue-600" : "text-gray-600"}`, children: [_jsx(FiUsers, { className: "text-xl mb-1" }), _jsx("span", { className: "text-xs", children: t('dashboard.operations.tabs.staff', 'Staff') })] })] }) }), _jsxs("main", { className: "flex-1 p-2 sm:p-3 md:p-6 overflow-hidden pb-16 md:pb-0", children: [tab === "overview" && (_jsx(OperationsOverview, { ref: operationsOverviewRef, metricsData: metricsData, isLoading: isLoadingMetrics, onRefresh: handleMetricsUpdate })), tab === "monitoring" && _jsx(ResourceMonitoring, {}), tab === "performance" && _jsx(PerformanceAnalytics, {}), tab === "staff" && _jsx(StaffManagement, {})] })] })] }));
}
