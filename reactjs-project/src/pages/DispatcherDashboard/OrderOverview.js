...
export default function StatsCards({ refreshTrigger }) {
    ...
    const stats = [
        { label: t('dashboard.dispatcher.orders.totalShipments'), value: totalShipments, icon: _jsx(PackageOpen, { size: 28, color: "#6366f1" }) }, // Indigo
        { label: t('dashboard.dispatcher.orders.totalVehicles'), value: totalVehicles, icon: _jsx(Truck, { size: 28, color: "#10b981" }) }, // Green
        { label: t('dashboard.dispatcher.orders.pendingPackages'), value: pendingPackages, icon: _jsx(Hourglass, { size: 28, color: "#f59e42" }) }, // Orange
        { label: t('dashboard.dispatcher.orders.packagesDelivered'), value: deliveredPackages, icon: _jsx(CheckCircle, { size: 28, color: "#22c55e" }) }, // Emerald
    ];
    if (loading) {
        return _jsx("div", { className: "mb-6", children: t('common.loading') });
    }
    if (error) {
