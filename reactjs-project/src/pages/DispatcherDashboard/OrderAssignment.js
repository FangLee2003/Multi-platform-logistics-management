...
export default function OrdersAssignment(_props) {
    ...
    const data = (Array.isArray(ordersPage?.data) ? ordersPage.data : []).map((item) => {
        ...
        if (addressValue && typeof addressValue === 'object') {
            addressDetail = {
                contactName: addressValue.contactName,
                contactPhone: addressValue.contactPhone,
            };
            addressField = addressValue; // truyền nguyên object
            // Debug log contact info extraction
            if (import.meta.env.DEV) {
                console.log('🔍 OrderAssignment: extracted contactName:', addressValue.contactName);
                console.log('🔍 OrderAssignment: extracted contactPhone:', addressValue.contactPhone);
        ...
        return {
            ...
            to: (orderItem.to || orderItem.toAddress || (typeof addressValue === 'object' && addressValue !== null && 'address' in addressValue ? addressValue.address : addressValue) || ""),
    ...
    return (_jsxs(_Fragment, { children: [_jsx(OrderDetailModal, { open: detailOpen, onClose: () => setDetailOpen(false), orderItem: detailOrder ? {
                    ...
                    addressDetail: (detailOrder && detailOrder.addressDetail)
                        ...
                        : (typeof detailOrder.address === 'object' && detailOrder.address !== null)
                            ? {
                                contactName: detailOrder.address.contactName,
                                contactPhone: detailOrder.address.contactPhone,
                            }
                            : undefined,
                ...
                } }), _jsxs("div", { className: "bg-gradient-to-br from-blue-50/80 via-white/80 to-blue-100/80 backdrop-blur-2xl rounded-3xl p-8 border border-white/40 shadow-2xl max-w-full overflow-x-auto", children: [_jsx("div", { className: "flex flex-col md:flex-row items-center justify-between mb-8 gap-4", children: _jsxs("div", { className: "flex items-center gap-4", children: [_jsx("span", { className: "inline-flex items-center justify-center w-12 h-12 rounded-full bg-blue-100 shadow-lg", children: _jsx(FaUserCog, { className: "text-3xl text-blue-600" }) }), _jsxs("div", { children: [_jsx("h3", { className: "text-3xl font-extrabold text-gray-900 tracking-tight", children: t('dashboard.dispatcher.assignment.title', 'Order Assignment Management') }), _jsxs("p", { className: "text-gray-600 mt-1", children: [t('common.total'), " ", totalOrders, " ", t('dashboard.dispatcher.orders.title', 'orders')] })] })] }) }), loading ? (_jsxs("div", { className: "text-center py-16 text-gray-500 text-lg animate-pulse", children: [t('common.loading'), "..."] })) : error ? (_jsxs("div", { className: "text-center py-12 px-4 bg-red-100/80 border border-red-200 rounded-xl text-red-700 font-semibold shadow flex items-center justify-center gap-2", children: [_jsx(FaTimes, { className: "text-xl text-red-500" }), error?.message || t('common.error')] })) : (_jsxs(_Fragment, { children: [successMessage && (_jsxs("div", { className: "mb-6 p-4 bg-green-100/90 border border-green-300 rounded-xl text-green-900 flex items-center gap-3 shadow-lg animate-fade-in", children: [_jsx(FaCheck, { className: "text-2xl text-green-600" }), _jsx("span", { className: "font-semibold text-base", children: successMessage })] })), _jsx("div", { className: "bg-white/70 backdrop-blur-xl rounded-2xl border border-white/60 overflow-x-auto shadow-xl", children: _jsx("div", { className: "overflow-x-auto", children: _jsxs("table", { className: "w-full min-w-[900px]", children: [_jsx("thead", { className: "sticky top-0 z-10", children: _jsxs("tr", { className: "bg-gradient-to-r from-blue-100/80 via-white/80 to-blue-50/80 border-b border-blue-200/60 shadow-sm", children: [_jsx("th", { className: "text-left p-5 font-bold text-gray-900 tracking-wide", children: t('dashboard.dispatcher.assignment.headers.orderId', 'Mã đơn') }), _jsx("th", { className: "text-left p-5 font-bold text-gray-900 tracking-wide", children: t('dashboard.dispatcher.assignment.headers.products', 'Sản phẩm') }), _jsx("th", { className: "text-left p-5 font-bold text-gray-900 tracking-wide", children: t('dashboard.dispatcher.assignment.headers.customer', 'Khách hàng') }), _jsx("th", { className: "text-left p-5 font-bold text-gray-900 tracking-wide", children: t('dashboard.dispatcher.assignment.headers.route', 'Lộ trình') }), _jsx("th", { className: "text-left p-5 font-bold text-gray-900 tracking-wide", children: t('dashboard.dispatcher.assignment.headers.orderDetails', 'Chi tiết đơn hàng') }), _jsx("th", { className: "text-left p-5 font-bold text-gray-900 tracking-wide", children: t('dashboard.dispatcher.assignment.headers.createdDate', 'Ngày tạo') }), _jsx("th", { className: "text-left p-5 font-bold text-gray-900 tracking-wide", children: t('dashboard.dispatcher.assignment.headers.vehicleDriver', 'Xe & Tài xế') }), _jsx("th", { className: "text-left p-5 font-bold text-gray-900 tracking-wide", children: t('dashboard.dispatcher.assignment.headers.actions', 'Thao tác') })] }) }), _jsx("tbody", { children: paginatedData.map((order, index) => (_jsxs("tr", { className: `border-b border-blue-100/40 hover:bg-blue-50/40 transition-all duration-200 ${index % 2 === 0 ? 'bg-white/60' : 'bg-blue-50/30'}`, children: [_jsx("td", { className: "p-5 align-top", children: _jsxs("div", { className: "flex items-center gap-2", children: [_jsx("span", { className: "font-bold text-lg text-blue-900", children: order.code }), _jsx("span", { className: `px-3 py-1 rounded-full text-xs font-bold border shadow-sm ml-2
                            ...
                            `, children: order.status })] }) }), _jsx("td", { className: "p-5 align-top min-w-[200px]", children: _jsx("div", { className: "max-w-xs font-bold text-blue-900 text-lg", children: typeof productCounts[order.id] === "number"
                                                                ...
                                                                _jsx("div", { className: "space-y-2", children: selectedVehicles[order.id] ? (
                                                                    ...
                                                                    _jsxs("select", { className: "w-full px-4 py-3 bg-white/90 border border-blue-200 rounded-xl focus:ring-2 focus:ring-blue-400 focus:border-blue-400 text-base shadow-sm font-medium text-blue-900", value: selectedVehicles[order.id] || "", onChange: (e) => handleVehicleSelect(order.id.toString(), e.target.value), children: [_jsx("option", { value: "", children: t('dashboard.dispatcher.assignment.selectVehicle', 'Select vehicle...') }), vehicles
                                                                                ...
                                                                                .map(vehicle => (_jsxs("option", { value: vehicle.id, children: [vehicle.licensePlate, " - ", vehicle.currentDriver?.fullName || t('dashboard.dispatcher.assignment.unknownDriver', 'Không rõ tài xế')] }, vehicle.id)))] })) })) }) }), _jsx("td", { className: "p-5 align-top min-w-[160px]", children: (!order.assignedVehicle && !order.currentDriver && selectedVehicles[order.id]) ||
                                                                (editingOrders[order.id] && selectedVehicles[order.id]) ? (_jsxs("div", { className: "flex gap-3", children: [_jsx("button", { onClick: () => handleAssignVehicle(order.id.toString()), disabled: assigningOrders[order.id], className: "flex items-center justify-center w-10 h-10 bg-green-500 hover:bg-green-600 disabled:bg-green-300 text-white rounded-full shadow-lg transition-all duration-200 text-lg font-bold focus:outline-none focus:ring-2 focus:ring-green-400", title: editingOrders[order.id] ? t('dashboard.dispatcher.assignment.updateVehicle', 'Update vehicle') : t('dashboard.dispatcher.assignment.assignVehicle', 'Assign vehicle'), children: assigningOrders[order.id] ? (_jsx("div", { className: "w-5 h-5 border-2 border-white border-t-transparent rounded-full animate-spin" })) : (_jsx(FaCheck, { className: "text-lg" })) }), _jsx("button", { onClick: () => editingOrders[order.id] ? handleCancelEdit(order.id.toString()) : handleCancelVehicleAssignment(order.id.toString()), className: "flex items-center justify-center w-10 h-10 bg-red-500 hover:bg-red-600 text-white rounded-full shadow-lg transition-all duration-200 text-lg font-bold focus:outline-none focus:ring-2 focus:ring-red-400", title: "H\u1EE7y", children: _jsx(FaTimes, { className: "text-lg" }) })] })) :
                                                                /* Hiển thị trạng thái đã gán với nút Unassign */
                                                                (order.assignedVehicle || order.currentDriver) && !editingOrders[order.id] ? (_jsxs("div", { className: "flex items-center gap-3", children: [_jsxs("div", { className: "flex items-center text-green-700 font-bold", children: [_jsx(FaCheck, { className: "text-xl" }), _jsx("span", { className: "ml-2 text-base", children: t('dashboard.dispatcher.assignment.assigned', 'Đã gán') })] }), _jsx("button", { onClick: () => handleUnassignVehicle(order.id.toString()), disabled: assigningOrders[order.id], className: "flex items-center justify-center w-9 h-9 bg-red-500 hover:bg-red-600 disabled:bg-red-300 text-white rounded-full shadow-lg transition-all duration-200 text-base font-bold focus:outline-none focus:ring-2 focus:ring-red-400", title: t('dashboard.dispatcher.assignment.unassignVehicle', 'Gỡ gán xe'), children: assigningOrders[order.id] ? (_jsx("div", { className: "w-4 h-4 border-2 border-white border-t-transparent rounded-full animate-spin" })) : (_jsx(FaTimes, { className: "text-base" })) })] })) : null })] }, order.id))) })] }) }) }), totalPages > 1 && (_jsxs("div", { className: "flex flex-col sm:flex-row justify-center items-center gap-4 mt-8", children: [_jsxs("button", { onClick: () => setCurrentPage(p => Math.max(1, p - 1)), disabled: currentPage === 1, className: "px-4 py-2 rounded-xl bg-blue-100 hover:bg-blue-200 disabled:opacity-50 disabled:cursor-not-allowed text-blue-700 font-bold shadow transition-all duration-150", children: ["< ", t('common.previous')] }), _jsx("div", { className: "hidden sm:flex items-center gap-2", children: Array.from({ length: Math.min(5, totalPages) }, (_, i) => {
                                            let pageNum;
                                            if (totalPages <= 5) {
                                                pageNum = i + 1;
                                            }
                                            ...
                                            return (_jsx("button", { onClick: () => setCurrentPage(pageNum), className: `w-10 h-10 rounded-lg font-bold transition-all duration-150 ${currentPage === pageNum
                                                    ? 'bg-blue-600 text-white shadow-lg'
                                                    : 'bg-white hover:bg-blue-50 text-blue-700 border border-blue-200'}`, children: pageNum }, pageNum));
                                        }) }), _jsxs("button", { onClick: () => setCurrentPage(p => Math.min(totalPages, p + 1)), disabled: currentPage === totalPages, className: "px-4 py-2 rounded-xl bg-blue-100 hover:bg-blue-200 disabled:opacity-50 disabled:cursor-not-allowed text-blue-700 font-bold shadow transition-all duration-150", children: [t('common.next'), " >"] })] })), _jsxs("div", { className: "text-center mt-4 text-gray-600", children: [t('dashboard.dispatcher.pagination.showing', 'Showing {{start}}-{{end}} of {{total}} orders', {
                                        start: data.length > 0 ? ((currentPage - 1) * PAGE_SIZE + 1) : 0,
                                        end: data.length > 0 ? ((currentPage - 1) * PAGE_SIZE + data.length) : 0,
                                        total: totalOrders
                                    }), totalPages > 1 && (_jsxs("span", { className: "ml-2", children: ["| ", t('dashboard.dispatcher.pagination.page', 'Page {{current}} / {{total}}', { current: currentPage, total: totalPages })] }))] }), data.length === 0 && (_jsxs("div", { className: "text-center py-16 text-gray-500", children: [_jsx(FaUserCog, { className: "text-5xl mx-auto mb-4 opacity-40" }), _jsx("p", { className: "text-xl font-semibold", children: t('dashboard.dispatcher.assignment.noOrders', 'No orders waiting for assignment') })] }))] }))] })] }));
}
