  ...
  const handleTrackingSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        setTrackingResult({
          code: order.orderId,
          status: order.status || t('common.unknown'),
    ...
    <div className="flex flex-col min-h-screen w-full bg-gray-50">
      ...
      <header className="fixed top-0 left-0 right-0 z-50 bg-green-700 shadow-md bg-opacity-90 backdrop-blur-md h-16 flex items-center px-4 sm:px-6 lg:px-8">
        <div className="flex justify-between items-center w-full">
          <span className="ml-2 text-xl font-bold text-white">{t('home.brand')}</span>
      ...
      <main className="flex-1 flex flex-col items-center justify-start mt-16 px-4 sm:px-6 lg:px-12 py-8 sm:py-12 bg-white">
          ...
          <form onSubmit={handleTrackingSubmit} className="space-y-4">
            <div className="flex flex-col sm:flex-row gap-4">
              <input
                ...
                placeholder={t('common.trackingPlaceholder')}
              ...
              >
                {isLoading ? t('common.trackingLoading') : t('common.trackingButton')}
              </button>
            </div>
          </form>
          {trackingResult && (
            <div className="mt-8 p-6 bg-green-50 border border-green-200 rounded-lg text-sm sm:text-base">
              <h3 className="text-lg font-semibold text-green-800 mb-4">
                {t('common.trackingInfo')}: {trackingResult.code}
              </h3>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <Info label={t('common.status')} value={trackingResult.status} />
                <Info
                  label={t('common.estimatedDelivery')}
                  value={formatDate(trackingResult.estimatedDelivery)}
                />
                <Info label={t('common.from')} value={trackingResult.from} />
                <Info label={t('common.to')} value={trackingResult.to} />
              </div>
            </div>
          )}
        </div>
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 sm:gap-8 mb-12 w-full">
          <Feature
            ...
            text={t('home.featureFastDesc')}
          />
          <Feature
            icon="📍"
            title={t('home.featureTracking')}
            text={t('home.featureTrackingDesc')}
          />
          <Feature
            icon="🔒"
            title={t('home.featureSecure')}
            text={t('home.featureSecureDesc')}
          />
        </div>
      ...
      <footer className="bg-black/80 text-white py-12 mt-16 rounded-t-2xl backdrop-blur-md">
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <FooterCol
            title={t('home.brand')}
            items={[t('footer.footerBrandDesc')]}
            highlight
          />
          <FooterCol
            title={t('footer.footerService')}
            items={[t('footer.footerService1'), t('footer.footerService2'), t('footer.footerService3')]}
          />
          <FooterCol
            title={t('footer.footerSupport')}
            items={[t('footer.footerHotline'), t('footer.footerEmail'), t('footer.footerFAQ')]}
          />
          <FooterCol
            title={t('footer.footerFollow')}
            items={[t('footer.footerFacebook'), t('footer.footerInstagram'), t('footer.footerLinkedIn')]}
          />
        </div>
        <div className="border-t border-gray-700 mt-8 pt-8 text-center text-gray-400 text-xs sm:text-sm">
          &copy; 2025 Fast Route. {t('footer.footerCopyright')}
        </div>
      </footer>
    </div>
  );
